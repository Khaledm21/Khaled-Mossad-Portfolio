...
function StatCard({ icon: Icon, value, label, color, index }) {
  return (
    <motion.div
      ...
      className="flex flex-col items-center text-center p-6 rounded-2xl border border-white/[0.06] bg-white/[0.02] hover:border-primary/20 transition-all duration-300 group hover:-translate-y-1 hover:shadow-[0_8px_30px_rgba(0,0,0,0.15)]"
    >
      {Icon && (
        <Icon 
          className="text-3xl mb-4 transition-all duration-300 group-hover:scale-110" 
          style={{ 
            color: color, 
      ...
      <div
        className="font-display font-extrabold text-[36px] leading-none mb-2"
        style={{ color }}
      >
        {value}
      </div>
...
function AboutCard({ icon: Icon, title, desc, color, index }) {
  return (
    <motion.div
      custom={index}
      variants={{
      ...
      <GlowCard glowColor={color} className="h-full p-[30px]">
        {Icon && (
          <div className="mb-6 flex justify-start items-center">
            <Icon 
              className="text-5xl transition-all duration-300 hover:scale-110 cursor-pointer"
              style={{ 
                color: color,
